...
1. Maven坐标
   ...
   - groupId：组织标识（包名）
   - artifactId：项目名称（项目名-模块名，例如spring-core，spring-beans等）
   - version：项目当前的版本
   ...
   那么jar包的位置为本地仓库中：`org/springframework/spring-webmvc/4.3.18.RELEASE/spring-webmvc-2.3.18.RELEASE.jar`

2. 依赖范围

   依赖范围scope用来控制依赖jar包和编译，测试，运行的classpath的关系，主要取值如下：

   - compiler：默认编译依赖范围，对于编译，测试，运行三种classpath都有效
   ...
   Maven声明周期就是为了对所有的构建过程进行抽象和统一，包括项目清理，初始化，编译，测试，打包，部署等构建过程。

   Maven的三套相互独立的生命周期，分别是：
